...
//             };
// //  probem is state for tail is not being deleted correctly
//             if (maximizingPlayer) {
//                 printf("Maximizing:\n");
//                 std::cout << "Avail moves: " << moves.size() << std::endl;
...
//             Grid enemyGrid = grid;
//             const int enemyAvailableSquares = this->floodFill(state.enemies[enemyIndex].head, enemyGrid, 0, true);
//             const float enemyPercentAvailable = (float) enemyAvailableSquares / (float) (this->m_width * this->m_height);

//             if (enemyAvailableSquares <= state.enemies[enemyIndex].length) {
//                 score += 9999999;
//             }
...
//             printf("grid element is: %d\n", grid[south.x][south.y]);
